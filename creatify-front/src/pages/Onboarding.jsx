    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="flex flex-col items-center px-4 py-10">
        <div className="w-full max-w-lg">
          ...
          {step === 1 && (
            <div>
              <h1 className="text-3xl font-black text-gray-900 mb-2">
                Bonjour ! Tu as <span className="text-emerald-600">quel âge</span> ?
              </h1>
              <p className="text-gray-400 text-sm mb-8">
                Pas besoin de compte. On veut juste te proposer des activités faites pour toi.
              </p>
                  ...
                  <button
                    ...
                    className={`border-2 rounded-xl p-4 text-center transition-all duration-150 ${
                      age === a.value
                        ...
                        : 'border-gray-200 bg-white hover:border-emerald-400 hover:bg-emerald-50 hover:shadow-sm'
                    }`}
                  >
          ...
          {step === 2 && (
            <div>
              <button onClick={() => setStep(1)} className="text-gray-400 text-sm mb-5 flex items-center gap-1 hover:text-gray-600 transition-colors">
                ← Retour
              </button>
              <h1 className="text-3xl font-black text-gray-900 mb-2">
                Qu'est-ce que tu <span className="text-emerald-600">aimes</span> faire ?
              </h1>
              <p className="text-gray-400 text-sm mb-8">Choisis autant que tu veux.</p>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {gouts.map(g => (
                  <button
                    key={g.value}
                    onClick={() => toggleGout(g.value)}
                    className={`border-2 rounded-xl p-3 flex items-center gap-3 transition-all duration-150 text-left ${
          ...
          {step === 3 && (
            <div>
              <button onClick={() => setStep(2)} className="text-gray-400 text-sm mb-5 flex items-center gap-1 hover:text-gray-600 transition-colors">
                ← Retour
              </button>
              <h1 className="text-3xl font-black text-gray-900 mb-2">
                Comment tu <span className="text-emerald-600">utilises</span> tes mains ?
              </h1>
              <p className="text-gray-400 text-sm mb-8">Pas de bonne ou mauvaise réponse.</p>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {motrics.map(m => (
                  <button
                    key={m.value}
